...
functions {
  ...
  int useVI) {
    ...
    for (i in 1:size(n_slice)) {
      ...
      real mu = intercept
      + b_ndvi*past_ndvi[n] 
      + (b_weight+ b_ndvi_weight*past_ndvi[n])*past_weight[n] 
      + (b_aggro+ b_ndvi_aggro*past_ndvi[n])*past_aggro[n] 
      + (b_reci+ b_ndvi_reci*past_ndvi[n])*past_reci[n] 
      + (b_trans+ b_ndvi_trans*past_ndvi[n])*past_trans[n] 
      + (b_trans_agg+ b_ndvi_trans_agg*past_ndvi[n])*past_trans_agg[n] 
      + (b_in_modu + b_ndvi_in_modu*past_ndvi[n])*past_in_modu[n]
      + troopID[troop[n]]
...
model {
  ...
  b_ndvi ~ normal(0,1) ;
  b_ndvi_aggro ~ normal(0,1) ; 
  b_ndvi_weight ~ normal(0,1) ;
  b_ndvi_reci ~ normal(0,1) ;
  b_ndvi_trans ~ normal(0,1) ;
  b_ndvi_trans_agg ~ normal(0,1);
  b_ndvi_in_modu ~ normal(0,1) ;
